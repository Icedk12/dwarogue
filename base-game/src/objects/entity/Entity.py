...
class Entity:
    ...
    def move_and_return(self, direction_vec2):
        ...
        new_x = self.position.x + direction_vec2.x
        new_y = self.position.y + direction_vec2.y

        new_position = pygame.Vector3(new_x, new_y, self.position.z)
        del new_x, new_y # Delete those placeholder variables because we don't need them anymore


        if self.game.map_manager.is_walkable(new_position):
        ...
        for z_layer in range(1, self.max_climb_height + 1):
            new_position.z = self.z + z_layer # Get the new z layer to step to
    ...
    def draw(self, scale=1.0):
        if self.position.z != self.game.camera_manager.z:
            return # Not on same z level as camera dont draw

        # Stores the position of the entity in the world on the tile grid
        world_position = pygame.Vector2(self.x * self.game.map_manager.tile_size, self.y * self.game.map_manager.tile_size)
